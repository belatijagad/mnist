...
class BaseTrainer:
    ...
    def __init__(self, model, criterion, metric_ftns, optimizer, config):
        ...
        # Configuration to monitor model and save the best
        if self.monitor == 'off':
            self.mnt_mode = 'off'
            self.mnt_best = 0
        else:
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            # Save logged information to the screen
            log = {'epoch': epoch}
            log.update(result)
            ...
            if self.mnt_mode != 'off':
                ...
                if not_improved_count > self.early_stop:
                    self.logger.info(f'Validation didn\'t improve for {self.early_stop} epochs.\nTraining stops.')
                    break
    ...
    def _save_checkpoint(self, epoch, save_best=False):
        ...
        filename = str(f'{self.checkpoint_dir}/checkpoint-epoch{epoch}.pth')
